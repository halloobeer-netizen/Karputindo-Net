...
import { Banknote, CalendarDays, CheckCircle2, ChevronLeft, ChevronRight, Clock3, Loader2, RefreshCw, Search, ShieldOff, WalletCards } from 'lucide-react';
...
type Data = { invoices:Invoice[]; stats:{total:number;unpaid:number;overdue:number;paid:number;revenue:number}; period:string; pagination:{page:number;pageSize:number;total:number;totalPages:number} };
...
export default function BillingPage() {
  ...
  const action=async(invoice:Invoice,a:string)=>{
    ...
    try {
      ...
      setData(current=>{
        ...
        const invoices=current.invoices.map(item=>item.id===invoice.id?{...item,status:nextStatus,customer:{...item.customer,serviceStatus:nextService}}:item);
      ...
      // Sinkronkan pelan-pelan di belakang tanpa mengosongkan tabel.
      void load(false);
  ...
  return <div className="space-y-6 p-6">
    <div className="flex flex-wrap items-center justify-between gap-4">
      <div><p className="text-sm text-gray-500">Keuangan & layanan pelanggan</p><h1 className="text-2xl font-bold text-gray-950">Billing & Isolir</h1><p className="mt-1 text-sm text-gray-500">Tagihan bulanan, jatuh tempo dan simulasi isolir MikroTik.</p></div>
